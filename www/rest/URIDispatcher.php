...
class URIDispatcher {
	...
	*/
	private function match_request($http_method, $url_pattern, &$matched_parameters = array()) {
		...
		/**
		 * $_SERVER['REQUEST_URI']: /rest/post/1
		 * $_SERVER['PHP_SELF']: /rest/index.php
		...
		/**
		 * Convert an string in an array of strings.
		 * If:
		 *   $path = "/post/1"
		 *   $pattern = "/post/$1"
		 * Then Tokens:
		 *   $pathTokens    = ["", "post", "1"]
		 *   $patternTokens = ["", "post", "$1"]
		 */
		...
		/**
         * Compare each URL segment with the corresponding pattern segment.
         * Literal segments must match exactly. Pattern segments using $1, $2, …
         * are treated as placeholders and capture the value from the URL.
         * If a literal segment does not match and is not a placeholder,
         * the route is considered not to match.
		 */
		$i = 0;
		$matched_parameters = array();
		for ($i = 0; $i < sizeof($pathTokens); $i++) {
			// If they do not match, catches the real value or the parameter (eg: post/$1 -> post/3)
		...
		// Parameters are sortby number to be passed to the callback
