	...
	public function findByIdWithAll($projectid){
		$stmt = $this->db->prepare("SELECT
			...
			if (sizeof($project_with_all) > 0) {
				...
				$project = new Project($project_with_all[0]["project.id"],
				$project_with_all[0]["project.name"]);

				$tasks_array = array();
				if ($project_with_all[0]["task.id"]!=null) {
					foreach ($project_with_all as $task){
						$task = new Task( $task["task.id"],
							$task["task.name"],$task["project.id"],
							$task["task.status"]
						);
		...
		*/
		public function save(Project $project) {
			...
			foreach ($project->getUsers() as $user) {
					$stmt->execute(array($users->getUserMail(),$project->getId()));
			}
